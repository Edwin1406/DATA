                            ...
                            <script>
                                (function() {
                                        ...
                                        function forceChoicesSelection(selectEl, values) {
                                            ...
                                            // A) Si el select está envuelto por el DIV .choices (instancia previa), sácalo y elimina el wrapper
                                            //    (esto funciona con temas que auto-inicializan Choices al cargar la página)
                                            const wrapper = selectEl.parentElement && selectEl.parentElement.classList.contains('choices') ?
                                                selectEl.parentElement :
                                                (selectEl.closest('.choices') && selectEl.closest('.choices') !== selectEl ? selectEl.closest('.choices') : null);

                                            if (wrapper && wrapper !== selectEl) {
                                                // mueve el <select> fuera del wrapper y remueve el wrapper
                                                wrapper.parentNode.insertBefore(selectEl, wrapper);
                                                wrapper.remove();
                                            }

                                            // B) Aplica la selección de forma nativa
